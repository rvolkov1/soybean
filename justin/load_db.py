...
def load_soybeans(conn, path):
    ...
    with conn.cursor() as cur, open(path, newline="", encoding="utf-8") as f:
        ...
        for row in rd:
            ...
            geofips = row[0].strip()
            region = row[1].strip()
            if not geofips or geofips.lower() == "geofips": continue
            cid = get_or_create_county(cur, geofips, region)
            for i, val in enumerate(row[2:]):
                y = years[i]
                if not y or not val or val in ("", "(NA)", "NA"): continue
                try:
                    v = float(val)
                    yi = int(y)
                except:
                ...
                cur.execute(
                    "INSERT INTO agricultural(county_year_id,soybean_yield_bu_acre) VALUES(%s,%s) ON CONFLICT (county_year_id) DO UPDATE SET soybean_yield_bu_acre=EXCLUDED.soybean_yield_bu_acre",
                    (cyid, v),
                )
    conn.commit()
...
def load_gdp(conn, path):
    ...
    with conn.cursor() as cur, open(path, newline="", encoding="utf-8") as f:
        ...
        for row in rd:
            ...
            geofips = row[0].strip()
            region = row[1].strip()
            if region in ("United States", ""): continue
            if not any(s in region for s in ("County", "Parish", "Borough", "Census Area")): continue
            if not geofips or geofips == "0": continue
            cid = get_or_create_county(cur, geofips, region)
            for i, val in enumerate(row[2:]):
                y = years[i]
                if not y or not val or val in ("", "(NA)", "NA", "(D)"): continue
                try:
                    v = float(val)
                    yi = int(y)
                except:
                ...
                cyid = ensure_cy(cur, cid, yi)
                cur.execute(
                    "INSERT INTO economy(county_year_id,total_gdp) VALUES(%s,%s) ON CONFLICT (county_year_id) DO UPDATE SET total_gdp=EXCLUDED.total_gdp",
                    (cyid, v),
                )
    conn.commit()
...
def load_weather(conn, path):
    ...
    with conn.cursor() as cur, open(path, newline="", encoding="utf-8") as f:
        ...
        for row in rd:
            ...
            try:
                y = int(d[:4])
                p = row.get("precip_mm"); ta = row.get("tavg_C"); tx = row.get("tmax_C"); tn = row.get("tmin_C")
                vals = [p, ta, tx, tn]
            ...
            cur.execute(
                "INSERT INTO weather(county_year_id,date,precip_mm,tavg_c,tmax_c,tmin_c) VALUES(%s,%s,%s,%s,%s,%s) ON CONFLICT (county_year_id,date) DO UPDATE SET precip_mm=EXCLUDED.precip_mm,tavg_c=EXCLUDED.tavg_c,tmax_c=EXCLUDED.tmax_c,tmin_c=EXCLUDED.tmin_c",
